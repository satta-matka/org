...
$(document).ready(function () {
    ...
    function processMumbaiGames () {
        ...
        for (var gameId in mumbaiGames) {
            var [gamename, time1, time2, a1a2a3, a, a4a5a6, timeBefore, timeAfter] = mumbaiGames[gameId];
            // if supplied timeout is null consider default
            if (!timeBefore) timeBefore = timeoutAfter;
            
            if (!timeAfter) timeAfter = timeoutAfter;
            ...
            var comp2 = (currentDateMinutes > (preDateTime2Minutes - timeBefore)) && (currentDateMinutes < (preDateTime2Minutes + timeAfter));
            var comp1 = (currentDateMinutes > (preDateTime1Minutes - timeBefore)) && (currentDateMinutes < (preDateTime1Minutes + timeAfter));
            ...
            if (comp2 || comp1) {
                ...
                if (a4a5a6.trim() !== '') {
                    html +=             `<span class="livepana vert-center"> - ` + a4a5a6 + `</span>`;
                }
                        html += `</div>
                                <hr>
                            </div>`;
    ...
    function processDelhiGames () {
        ...
        for (var gameId in delhiGames) {
            var [gamename, time1, timeBefore] = delhiGames[gameId];
            // if supplied timeout is null consider default
            if (!timeBefore) timeBefore = timeoutAfter;
            // if we have the div we don't add it
            if ($('#live-update-' + gameId).length > 0) {
                expiry(gameId, timeBefore);
                continue;
            }
            ...
            var comp = (currentDateMinutes > (preDateTime1Minutes - timeBefore)) && (currentDateMinutes < (preDateTime1Minutes + timeBefore));
            if (comp) {
                // time1 is within range. so we should make it visible
                var html = `<div id="live-update-`+ gameId + `" class="update-item">
                                <div class="gamename-head">
                ...
                $('#live-updates').append(html);
                marqueAsArray.push(['delhi', gameId, gamename, timeBefore]);
                expiry(gameId, timeBefore);
            }
            // start loading in time
            if (comp) {
