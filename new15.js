    ...
    function processMarque () {
        ...
        if (toShow[0] === 'mumbai') {
            ...
            if ((toShow[3].trim() !== '') && toShow[4].length) {
                ...
                html += `<span class="pana-number"> Loading...</span>`;
            }
        ...
        for (var gameId in mumbaiGames) {
            ...
            if ($('#live-update-' + gameId).length > 0) {
                ...
                if (a4a5a6.trim() === '') {
                    ...
                    $('[data-span="live-span-' + gameId + '"]').html(`<span style="display: inline-flex;" data-span="live-span-` + gameId + `">`+ gamename +`</span>`);
                }
                continue;
            }
            ...
            if (comp2 || comp1) {
                ...
                if (a1a2a3.trim() === '') {
                    html += `
                                    <span class="pana-number" data-pana="">Loading...</span>
    	...
    	if (res !== 'Error') {
    		if (res.type == 'mumbai') {
    		  ...
    		  //  remove loader
    			...
    			var updateHtml = `
    		        <span class="jodi-number" data-pana="">` + res.a.join('') + `</span>
    		    `;
    // 			add realtime data
    			$('[data-update="live-update-' + res.target + '"]').html(updateHtml);
    // 			remove loader
    		    ...
    		    for (const gameId in delhiGames) {
    		        ...
    		        if (gameId === res.target) mumbaiGames[gameId] = [item[0], item[1], res.a];
    		    }
